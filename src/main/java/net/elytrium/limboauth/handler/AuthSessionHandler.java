...
public class AuthSessionHandler implements LimboSessionHandler {
    ...
    public AuthSessionHandler(RegisteredPlayerRepository registeredPlayerRepository, Player proxyPlayer, LimboAuth plugin, @Nullable RegisteredPlayer playerInfo) {
    ...
    public void onSpawn(Limbo server, LimboPlayer player) {
        ...
        if (this.playerInfo == null) {
            try {
                ...
                List<RegisteredPlayer> alreadyRegistered = this.registeredPlayerRepository.getByIp(ip);
        ...
        if (args.length != 0 && this.checkArgsLength(args.length)) {
            ...
            if (command == Command.REGISTER && !this.totpState && this.playerInfo == null) {
                ...
                if (this.checkPasswordsRepeat(args) && this.checkPasswordLength(password) && this.checkPasswordStrength(password)) {
                    ...
                    try {
                        this.registeredPlayerRepository.createIfNotExists(registeredPlayer);
                ...
                if (password.length() > 0 && checkPassword(password, this.playerInfo, this.registeredPlayerRepository)) {
    ...
    public static boolean checkPassword(String password, RegisteredPlayer player, RegisteredPlayerRepository repository) {
    ...
    public static RegisteredPlayer fetchInfo(RegisteredPlayerRepository playerDao, UUID uuid) {
        try {
            List<RegisteredPlayer> playerList = playerDao.getByPremiumUUID(uuid.toString());
    ...
    public static RegisteredPlayer fetchInfo(RegisteredPlayerRepository repository, String nickname) {
        try {
            return repository.getByLowercaseName(nickname.toLowerCase(Locale.ROOT)).orElse(null);
        } catch (DataAccessException e) {
            throw new DataAccessRuntimeException(e);
        }
    }

    public static RegisteredPlayer fetchInfoLowercased(RegisteredPlayerRepository repository, String nickname) {
        try {
            return repository.getByLowercaseName(nickname).orElse(null);
        } catch (DataAccessException e) {
            throw new DataAccessRuntimeException(e);
        }
    }
